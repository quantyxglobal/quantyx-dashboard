...
export function AdminCaseList({ cases }: AdminCaseListProps) {
  ...
  const { prefetchAdminCaseRoute } = usePrefetch()
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <div className="flex-1 min-w-[200px] space-y-2">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary text-base leading-snug"
            >
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredCases.map((caseItem) => (
                <tr 
                  key={caseItem.id}
                  onClick={() => router.push(`/admin/case/${caseItem.id}`)}
                  onMouseEnter={() => prefetchAdminCaseRoute(caseItem.id)}
